// ============================================================================
// PAINEL DE MATRÍCULAS 2026.2 - lógica principal
// ============================================================================

let DADOS = null;          // último payload recebido do Apps Script
let ABA_ATIVA = "presencial"; // "presencial" | "ead"
let GRAFICOS = {};         // referências das instâncias Chart.js, para destruir/redesenhar
...
function mostrarEstadoConfiguracaoPendente() {
  el("app").innerHTML = `
    <div class="estado-central">
      ...
      <p>Falta apenas colar a URL do Apps Script publicado no arquivo <b>config.js</b>.
      Siga o passo a passo do README para publicar o Web App e liberar os dados reais aqui.</p>
...
function registrosFiltrados() {
  ...
  return DADOS.registros.filter(r =>
    r.modalidadeGeral.toLowerCase() === ABA_ATIVA.replace("ead", "EAD").toLowerCase() ||
    (ABA_ATIVA === "ead" && r.modalidadeGeral === "EAD") ||
    (ABA_ATIVA === "presencial" && r.modalidadeGeral === "Presencial")
  ).filter(r =>
...
function trocarAba(aba) {
  ...
  el("tab-ead").classList.toggle("ativa", aba === "ead");
  renderizarTudo();
}
...
function renderizarCards(regs) {
  ...
  el("cards").innerHTML = `
    ...
    ${cardMeta("Meta do bloco (" + (ABA_ATIVA === "presencial" ? "Presencial" : "EAD") + ")", total, meta, pctMeta)}
...
function renderizarFunil(regs) {
  ...
  GRAFICOS.funil = new Chart(ctx, {
    type: "bar",
    data: {
      labels: ["Total de leads/matrículas", "Minutas (aguardando FIES)", "Matrículas pagas"],
      datasets: [{
        data: [total, minutas, pagas],
        backgroundColor: ["#4B2E83", "#F7931E", "#1E9E6B"],
        borderRadius: 8,
        maxBarThickness: 60,
      }]
    },
...
function renderizarGraficoFinanceiro(regs) {
  ...
  GRAFICOS.financeiro = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ data: grupos.map(g => g.total), backgroundColor: ["#4B2E83", "#F7931E", "#8A8A9C"] }]
    },
...
function renderizarConsultores(regs) {
  ...
  el("consultores-grid").innerHTML = nomes.map(nome => {
    ...
    return `<div class="consultor-card">
      <h4>${nome}</h4>
      <div class="consultor-metrica"><span>Matrículas</span><span>${doC.length}</span></div>
...
function renderizarAlertas() {
  ...
  if (inconsistencias.length === 0) {
    el("alertas").innerHTML = `<div class="alerta-ok">Nenhuma inconsistência encontrada na última leitura da planilha.</div>`;
    return;
  }
  el("alertas").innerHTML = inconsistencias.map(inc => `
    <div class="alerta"><b>${inc.tipo}</b> — linha ${inc.linha} (${inc.aluno || "sem nome"}). ${inc.detalhe}
    ${inc.valorOriginal ? ` Valor original na célula: "${inc.valorOriginal}".` : ""}</div>
  `).join("");
}
